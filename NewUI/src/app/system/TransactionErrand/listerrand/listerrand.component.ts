...
@Component({
  selector: 'app-listerrand',
  templateUrl: './listerrand.component.html',
  styleUrls: ['./listerrand.component.scss']
})
export class ListerrandComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getpagepermission(this.userId,11);
    this.getListOftable();
  ...
  view(id: number, button: boolean) {
    this.router.navigate(['/addOrEditerrand/' + id + '/' + button])
  }
  addOrEdit(id: number) {
    this.router.navigate(['/addOrEditerrand', id])
  }
  Delete(element: any) {
